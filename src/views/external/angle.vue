  ...
  <div class="root">
    ...
    <el-row :gutter="10">
      ...
      <el-col :span="6" style="text-align: right;">
          ...
          <el-button @click="handleSatelliteAngleQuery" slot="append" icon="el-icon-search"></el-button>
        </el-input>
			</el-col>
      
		</el-row>

    <el-table :data="angleList" style="width: 100%">
...
<script>
  ...
  //import mock from '../../../mock/external_angle'
  import { postGroundStations } from '@/api/api_groundstation'
  import { postSatelliteAngle, postSatelliteAngleByPage } from '@/api/api_external'
  export default {
    data() {
      return {
        satelliteid: "",
    ...
    methods: {
      ...
      queryByPage() {
        ...
        postSatelliteAngleByPage({pageSize, currentPage}).then(response => {
			    console.log(response)
          if (response.code === 200) {
            const page = response.respBody
            this.total = page.total
            this.currentPage = page.currentPage
            this.angleList = page.dataList
          }
		    })
      },
